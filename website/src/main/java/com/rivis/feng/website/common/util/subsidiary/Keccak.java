...
import com.rivis.feng.website.common.enums.SHAEnum;

import static com.rivis.feng.website.common.util.HexUtil.getReverseHex;
...
public class Keccak {
    ...
    public String getHash(String message, SHAEnum shaEnum) {
        ...
        BigInteger[][] P = padding(message, shaEnum);

        //	    Absorbing phase
        for (BigInteger[] Pi : P) {
            for (int i = 0; i < 5; i++) {
                for (int j = 0; j < 5; j++) {
                    if ((i + j * 5) < (shaEnum.getR() / w)) {
        ...
        do {
            ...
            doKeccackf(S);
        } while (Z.length() < shaEnum.getOutputLength() * 2);

        return Z.substring(0, shaEnum.getOutputLength() * 2);
    }
    ...
    private BigInteger[][] padding(String message, SHAEnum shaEnum) {
        int size;
        message = message + shaEnum.getD();

        while (((message.length() / 2) * 8 % shaEnum.getR()) != ((shaEnum.getR() - 8))) {
            message = message + "00";
        }

        message = message + "80";
        size = (((message.length() / 2) * 8) / shaEnum.getR());
